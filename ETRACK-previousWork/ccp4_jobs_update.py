...
class pdbcur_output:
	...
	def pdbCUR_run(self):
		...
		input1 = "/Applications/ccp4-6.4.0/bin/pdbcur "+\
				 "XYZIN %s " %(str(self.inputpdbfile))+\
				 "XYZOUT %s " %(str(self.outputpdbfile))
		...
		print '--------------------------'
		print 'Summary:'
		print 'Input pdb file: %s' %(str(self.inputpdbfile))
		print 'Output pdb file: %s' %(str(self.outputpdbfile))

		# determine initial number of atoms in input pdb file
		pdbin = open(str(self.inputpdbfile),'r')
		...
		pdbout = open(str(self.outputpdbfile),'r')
...
class SFALL_output:
	...
	def SFALL_run(sfall_GRID):
		...
		binarymapfile = open(str(self.outputmapfile),'rb')
		binarymapfile.seek(7*4,0)
		self.gridsamp1 = struct.unpack('=l',binarymapfile.read(4))[0]
		self.gridsamp2 = struct.unpack('=l',binarymapfile.read(4))[0]
		self.gridsamp3 = struct.unpack('=l',binarymapfile.read(4))[0]
		...
		print 'Summary:'
		print 'Input pdb file: %s' %(str(self.inputpdbfile))
		print 'Output map file: %s' %(str(self.outputmapfile))
		print 'Grid sampling 1: %s' %(str(self.gridsamp1))
		print 'Grid sampling 2: %s' %(str(self.gridsamp2))
		print 'Grid sampling 3: %s' %(str(self.gridsamp3))

		xyz = ['X','Y','Z']
		print 'Fast axis: %s' %(str(xyz[int(sfastaxis)-1]))
		print 'Medium axis: %s' %(str(xyz[int(medaxis)-1]))
		print 'Slow axis: %s' %(str(xyz[int(slowaxis)-1]))
		print '--------------------------'

		# get remaining attribute values
		self.fastaxis = str(xyz[int(fastaxis)-1])
		self.medaxis = str(xyz[int(medaxis)-1])
		self.slowaxis = str(xyz[int(slowaxis)-1])
...
class FFT_output:
	...
	def FFT_run(self,SFALL_runoutput,mtzlabels):
		...
		input1 = "/Applications/ccp4-6.4.0/bin/fft "+\
				 "HKLIN %s " %(str(self.inputmergedmtzfile))+\
			 	 "MAPOUT %s " %(str(self.outputmapfile))+\
			 	 "SYMINFO /Applications/ccp4-6.4.0/lib/data/syminfo.lib "\
		...
		# print a summary of resulting FFT map here
		self.printSummary()
		...
		def printSummary(self):
			...
			print '--------------------------'
			print 'Summary:'
			print 'Input merged mtz file: {}'.format(str(self.inputmergedmtzfile))
			print 'Output map file: {}'.format(str(self.outputmapfile))
			print 'Grid sampling 1: {}'.format(str(self.gridsamp1))
			print 'Grid sampling 2: {}'.format(str(self.gridsamp2))
			print 'Grid sampling 3: {}'.format(str(self.gridsamp3))

			xyz = ['X','Y','Z']
			print 'Fast axis: {}'.format(self.fastaxis)
			print 'Medium axis: {}'.format(self.medaxis)
			print 'Slow axis: {}'.format(self.slowaxis)
			print '--------------------------'
